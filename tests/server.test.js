...
const { runVendorBootstrap } = require("../src/lib/vendor-bootstrap");

jest.mock("../src/lib/vendor-bootstrap", () => ({
  runVendorBootstrap: jest.fn(async () => ({ ok: true, code: 0, output: "bootstrap complete", scriptPath: "/mock/bootstrap-yt-dlp.js" })),
...
test("GET /api/feeds returns feeds array", async () => {
  const res = await request(app).get("/api/feeds");
  expect(res.status).toBe(200);
  expect(Array.isArray(res.body.feeds)).toBe(true);
});

test("GET /api/diagnostics returns runtime and binaries", async () => {
  ...
  expect(res.body).toHaveProperty("writablePaths");
  expect(res.body).toHaveProperty("recentErrors");
  expect(Array.isArray(res.body.binaries)).toBe(true);
}, 30000);

test("POST /api/diagnostics/repair returns repair result", async () => {
  const res = await request(app).post("/api/diagnostics/repair");
  expect(res.status).toBe(200);
  expect(res.body).toMatchObject({ ok: true, code: 0 });
});
...
test("POST /api/download-queue/rerun rejects unknown task id", async () => {
  const res = await request(app)
    .post("/api/download-queue/rerun")
    .send({ taskId: "nonexistent", mode: "exact" });
