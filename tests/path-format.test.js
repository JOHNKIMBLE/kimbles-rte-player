...
describe("buildDownloadTarget", () => {
  ...
  test("supports host-aware path tokens", () => {
    const result = buildDownloadTarget({
      ...baseArgs,
      pathFormat: "{source_type}/{host_slug}/{hosts_slug}/{episode}",
      hosts: ["Jenny Greene", "Guest Host"]
    });
    expect(result.tokens.source_type).toBe("rte");
    expect(result.tokens.host).toBe("Jenny Greene");
    expect(result.tokens.host_slug).toBe("jenny-greene");
    expect(result.tokens.hosts).toBe("Jenny Greene, Guest Host");
    expect(result.tokens.hosts_slug).toBe("jenny-greene-guest-host");
    expect(result.outputDir).toMatch(/rte/i);
    expect(result.outputDir).toMatch(/jenny-greene/i);
  });
