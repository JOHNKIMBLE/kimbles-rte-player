...
describe("createDownloadQueue", () => {
  test("returns queue API", () => {
    ...
    expect(queue).toHaveProperty("restorePending");
  ...
  test("restores queued persistent tasks from disk", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "queue-restore-"));
    const storePath = path.join(tempDir, "queue.json");
    const firstQueue = createDownloadQueue(() => 1, {
      storagePath: storePath
    });
    firstQueue.pause();
    const pendingPromise = firstQueue.run(
      () => ({ outputDir: "/out", fileName: "original.mp3" }),
      { label: "Persist me", sourceType: "rte", persisted: { type: "test", value: 42 } }
    );
    expect(firstQueue.snapshot().queuedCount).toBe(1);

    const restoredValues = [];
    const secondQueue = createDownloadQueue(() => 1, {
      storagePath: storePath,
      restoreTask: (persisted) => ({
        run: () => {
          restoredValues.push(persisted.value);
          return { outputDir: "/out", fileName: "restored.mp3" };
        },
        meta: {
          label: "Persist me",
          sourceType: "rte",
    ...
    const restoredCount = secondQueue.restorePending();
    expect(restoredCount).toBe(1);
    secondQueue.resume();
    await delay(30);

    expect(restoredValues).toEqual([42]);
    expect(secondQueue.snapshot().recent[0].status).toBe("done");

    firstQueue.clearPending();
    await expect(pendingPromise).rejects.toThrow("Cancelled");
    fs.rmSync(tempDir, { recursive: true, force: true });
  });

  test("reruns a recent persistent task in exact and current-settings modes", async () => {
    const modes = [];
    const queue = createDownloadQueue(() => 1);
    await queue.run(
      () => ({ outputDir: "/out", fileName: "first.mp3" }),
      { label: "Persisted task", sourceType: "rte", persisted: { type: "test-job", value: 7 } }
    );
    await delay(10);

    const recentTaskId = queue.snapshot().recent[0].id;
    const exact = queue.rerun(recentTaskId, {
      mode: "exact",
      restoreTask: (persisted, _item, options = {}) => ({
        run: () => {
          modes.push(`exact:${persisted.value}:${options.mode}`);
          return { outputDir: "/out", fileName: "exact.mp3" };
        },
        meta: {
    ...
    const current = queue.rerun(recentTaskId, {
      mode: "current-settings",
      restoreTask: (persisted, _item, options = {}) => ({
        run: () => {
          modes.push(`current:${persisted.value}:${options.mode}`);
          return { outputDir: "/out", fileName: "current.mp3" };
        },
        meta: {
          label: "Persisted task",
          sourceType: "rte",
